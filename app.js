...
function validateLastImput(value) {
  ...
  if (operators.includes(lastCharacter) && !Number.isInteger(parseInt(value))) {
    // si el ultimo caracter se encuentra en la lista de operadores y el valor no es numerico retorna falso y de no ser seria valido
    //! es negacion 
    //inInteger funcion para validar si el parametro corresponde a un numerico
    // parseInt transformar el numero que viene en formato string y pasa a numerico para la validacion del dato
    return false;
  }
    return true;
}
